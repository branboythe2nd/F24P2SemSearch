...
public class ControllerTest extends student.TestCase {
    ...
    /**
     * Tests the insert for controller
     */
    public void testInsert() {
        ...
        assertEquals(test.getIdTree().getNumOfRecords(), 1); // Should still be
                                                             // 1 because
                                                             // insertion failed

        // Try inserting a Seminar with invalid coordinates (x or y out of
        // bounds)
        test.insert(new Seminar(12, "invalid", "2710051600", 120, (short)-1,
            (short)0, 3, keywords, "oo"), "4"); // x < 0
        assertEquals(test.getIdTree().getNumOfRecords(), 1); // Insertion should
                                                             // fail
        ...
        assertEquals(test.getIdTree().getNumOfRecords(), 2); // Now there should
                                                             // be 2 records
    }


    /**
     * Tests the controller search
     */
